...
export class IncomingTransactionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getSortedIncomingTransactions();
  }

  getSortedIncomingTransactions(){
      ...
      this.crudService.getTypeRequest('/accounts/user/' + this.userId).subscribe({
        next: (res) => {
          ...
          for (let i = 0; i < this.accounts.length; i++) {
            ...
            this.crudService.getTypeRequest('/transactions/account/' + this.accountRef + '/status/Incoming').subscribe({
              next: (res) => {
                this.sortedIncomingTransactions = res;
                // console.log(this.sortedIncomingTransactions);
